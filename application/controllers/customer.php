...
class Customer extends CI_Controller{
	...
	public function start(){
		$result = $this->customer_model->start();
	...
	public function register(){
		...
		$insert = $this->customer_model->register($data);
	...
	public function Edition($type){
		if($type == 'bringData'){
			...
			$sql = $this->customer_model->loadCustomer($id);
			
			foreach ($sql as $row) {
				$doc = $row['cli_documento'];
				$name = addslashes($row['cli_nombre']);
				$phone = $row['cli_telefono'];
				$direction = addslashes($row['cli_direccion']);
			}
